...
def ensure_user(user):
    ...
    conn = db()
    if existing is None:
        conn.execute("INSERT INTO users(user_id,name,username,joined_at) VALUES(?,?,?,?)", (uid, name, username, now))
    else:
        conn.execute("UPDATE users SET name=?, username=? WHERE user_id=?", (name, username, uid))
    conn.commit()
    conn.close()
...
def main_menu_markup():
    ...
    m.row(
        ...
        make_button(custom_menu_text("lucky"), callback_data="btn_ludo"),
    )
...
def start(message):
    ...
    if len(parts) == 2: payload = parts[1].strip()
    if payload.startswith("ref_"): payload = payload[4:]
    if payload.isdigit() and int(payload) != uid:
        conn = db()
        if not conn.execute("SELECT 1 FROM referrals WHERE user_id=?", (uid,)).fetchone():
...
def get_panel_items(panel):
    ...
    for app_code, product in CATALOG.items():
        if isinstance(product, dict) and product.get("_panel") == panel and app_code not in seen:
...
def show_panel(call, panel):
    title = {"pnl_nonroot": "ANDROID NON ROOT PANELS", "pnl_root": "ANDROID ROOT PANELS", "pnl_iphone": "IPHONE PANELS", "pnl_pc": "PC PANELS"}.get(panel, "PANELS")
...
def product_markup(app_code, stock):
    ...
    m = InlineKeyboardMarkup()
    if not product: return m
    for duration, price in product.items():
        if duration in ("name", "_panel"): continue
        label = duration_text(duration)
        if stock > 0:
            m.add(make_button(f"Buy {label} - {money(price)} (~ ${usd(price):.2f})", callback_data=f"buy_{app_code}_{duration}", style="success", emoji_key=f"app_{app_code}"))
        else:
            m.add(make_button(f"{label} (Out of Stock)", callback_data=f"oos_{app_code}_{duration}", style="danger", emoji_key=f"app_{app_code}"))
    m.add(make_button("BACK TO PANELS", callback_data=get_product_panel(app_code), style="danger", emoji_key="btn_back"))
    return m
...
def show_product(call, app_code):
    ...
    for duration, price in product.items():
        if duration in ("name", "_panel"): continue
        lines.append(f"🛒 <b>Validity:</b> {esc(duration_text(duration))}\n💰 Price: <b>{money(price)}</b> (~ ${usd(price):.2f})\n📱 Limit: 1 Device | 📦 <b>{status}</b>\n")
    lines.append("🛡️ <b>Select package below to purchase:</b>")
    bot.edit_message_text("\n".join(lines), call.message.chat.id, call.message.message_id, reply_markup=product_markup(app_code, stock))
...
def profile_text(uid):
    ...
    return (
        ...
        f"🆔 <b>ID:</b> <code>{uid}</code>\n👤 <b>Name:</b> {esc(row['name'] if row else 'User')}\n\n"
        f"💰 Current Balance: <b>{money(row['balance'] if row else 0)}</b>\n\n"
        f"🔑 Total Orders: <b>{orders['c']}</b>\n📈 Total Spent: <b>{money(orders['s'])}</b>\n"
        f"👥 Total Referrals: <b>{row['referrals'] if row else 0}</b>\n📅 Joined: <b>{esc(row['joined_at'] if row else '')}</b>"
    )

def orders_text(uid):
    conn = db()
    rows = conn.execute("SELECT * FROM orders WHERE user_id=? ORDER BY id DESC LIMIT 10", (uid,)).fetchall()
    conn.close()
    text = "<b>🧾 — YOUR RECENT ORDERS (LAST 10) — 🧾</b>\n━━━━━━━━━━━━━━━━━━━━\n\n"
    if not rows: return text + "No purchases yet!"
    for r in rows:
        text += f"📦 <b>{esc(r['product'])}</b>\n⏱️ {esc(r['duration'])} | {money(r['price'])}\n🔖 Order: <code>#{r['id']}</code> | <b>{esc(r['status'])}</b>\n📅 {esc(r['created_at'])}\n━━━━━━━━━━━━━━━━━━━━\n"
    return text

def balance_markup():
    m = InlineKeyboardMarkup()
    m.row(make_button("Paytm UPI", callback_data="btn_paytm_upi", style="success", emoji_key="btn_paytm_upi"), make_button("Binance Pay", callback_data="btn_binance_pay", style="primary", emoji_key="btn_binance_pay"))
...
def payment_quick_markup():
    m = InlineKeyboardMarkup()
    m.row(make_button("₹100", callback_data="pay_quick_100", style="success"), make_button("₹500", callback_data="pay_quick_500", style="success"))
    m.row(make_button("₹1000", callback_data="pay_quick_1000", style="success"), make_button("₹2000", callback_data="pay_quick_2000", style="success"))
...
def send_upi_payment(chat_id, amount):
    ...
    admin = clean_admin()
    text = f"<b>💳 PAYMENT DETAILS</b>\n\nAmount: <b>₹{amount}</b>\nUPI ID: <code>{esc(upi_id)}</code>\n\nScan QR & complete payment.\nSend proof to admin: @{esc(admin)}"
    bot.send_photo(chat_id, qr_url, caption=text, reply_markup=back_markup())
...
def admin_menu():
    ...
    m.row(make_button("Bot Settings", callback_data="admin_bot"), make_button("Button Labels", callback_data="admin_buttons"))
    m.row(make_button("Button Styles", callback_data="admin_styles"), make_button("Custom Emojis", callback_data="admin_emojis"))
    m.row(make_button("Messages", callback_data="admin_messages"), make_button("Payments", callback_data="admin_payment"))
    m.row(make_button("Products", callback_data="admin_products", style="success"), make_button("Users", callback_data="admin_users", style="success"))
    m.row(make_button("Orders", callback_data="admin_orders", style="success"), make_button("Tickets", callback_data="admin_tickets", style="success"))
    m.row(make_button("Referral", callback_data="admin_referral"), make_button("Reload", callback_data="admin_reload"))
...
def admin_command(message):
    ...
    bot.send_message(message.chat.id, "<b>⚙️ ADMIN CONTROL CENTER</b>", reply_markup=admin_menu())
...
def admin_callback(call):
    ...
    if data == "admin_home":
        bot.edit_message_text("<b>⚙️ ADMIN CONTROL CENTER</b>", chat_id, message_id, reply_markup=admin_menu())
        return
...
def normal_callback(call):
    ...
    if data == "btn_store": show_store(chat_id, message_id)
    elif data in PANEL_ITEMS: show_panel(call, data)
    elif data.startswith("app_"): show_product(call, data[4:])
    elif data.startswith("oos_"): bot.answer_callback_query(call.id, "Out of stock.", show_alert=True)
        ...
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        conn = db()
        cur = conn.execute("INSERT INTO orders(user_id,product,duration,price,status,created_at) VALUES(?,?,?,?,?,?)",
                           (uid, product.get("name", app_code), duration, float(price or 0), "PENDING", now))
        oid = cur.lastrowid
        conn.commit()
        conn.close()
        bot.send_message(chat_id, f"Order <code>#{oid}</code> created!", reply_markup=back_markup())

    elif data == "btn_profile":
        bot.edit_message_text(profile_text(uid), chat_id, message_id, reply_markup=back_markup())
        ...
        bot.edit_message_text("<b>💰 ADD BALANCE</b>", chat_id, message_id, reply_markup=balance_markup())
    elif data == "btn_paytm_upi":
        bot.edit_message_text("Select Payment Amount:", chat_id, message_id, reply_markup=payment_quick_markup())
    elif data.startswith("pay_quick_"):
        send_upi_payment(chat_id, int(data.replace("pay_quick_", "")))
...
def handle_ticket_message(message):
    ...
    conn.commit()
    conn.close()
    bot.send_message(message.chat.id, f"Ticket <code>#{tid}</code> created!", reply_markup=back_markup())
